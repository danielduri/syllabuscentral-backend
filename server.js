...
app.get("/", (req, res) => {
  res.send(`Server is up, ${process.env.NODE_ENV}`);
});
app.get("/userInfo", verifyToken, (req, res) => {
  userInfo(req, res, db);
});
app.get("/getModel", verifyToken, (req, res) => {
  getModel(req, res, db);
});
app.get("/getDegreeCoordinators", verifyToken, (req, res) => {
  getDegreeCoordinators(req, res, db);
});
app.get("/getDegrees", verifyToken, (req, res) => getDegrees(req, res, db));
app.get("/getDepartments", verifyToken, (req, res) =>
  getDepartments(req, res, db)
);
app.get("/getDegreeNames", verifyToken, (req, res) => {
  getDegreeNames(req, res, db);
});
app.get("/getModules", verifyToken, (req, res) => {
  getModules(req, res, db);
});
app.get("/getSubjects", verifyToken, (req, res) => {
  getSubjects(req, res, db);
});
app.get("/getSchools", verifyToken, (req, res) => {
  getSchools(req, res, db);
});
app.get("/getDepartmentNames", verifyToken, (req, res) => {
  getDepartmentNames(req, res, db);
});
...
app.put("/changePassword", verifyToken, (req, res) =>
  changePassword(req, res, db, bcrypt)
);
...
app.post("/newDepartment", verifyToken, (req, res) =>
  newDepartment(req, res, db)
);
app.put("/deleteDepartment", verifyToken, (req, res) =>
  deleteDepartment(req, res, db)
);
app.put("/editDepartment", verifyToken, (req, res) =>
  editDepartment(req, res, db)
);
...
app.put("/deleteSubject", verifyToken, (req, res) =>
  deleteSubject(req, res, db)
);
...
app.post("/recommendationsTest", verifyToken, (req, res) =>
  returnDummyArray(req, res)
);
    ...
    if (typeof msg === "string") {
      try {
        ...
        verifyUserInfo(user, db).then((user) => {
          ...
          filename =
            data.name.substring(0, data.name.length - (ext.length + 1)) +
            "-" +
            uniqueSuffix +
            "." +
            ext;
