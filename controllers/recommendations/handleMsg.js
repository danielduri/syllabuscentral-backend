...
export async function handleMsg(req, res) {
  ...
  const GPT35TurboMessage = [
    ...
    {
      role: "user",
      content: `Escribe sugerencias concretas para el campo ${field} de una guía docente para la asignatura ${title}
      (${degree}), ${completionMessage} en formato ${formatModel} crudo.`,
    },
  ];
  ...
  return openai
    ...
    .then((response) => {
      ...
      const cost =
        (response.data.usage.total_tokens * process.env.COST_PER_1000_TOKENS) /
        1000;
      console.log(
        "Cost: " + response.data.usage.total_tokens + " tokens, " + cost + " $"
      );
      res.json({
